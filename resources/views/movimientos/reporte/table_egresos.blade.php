        ...
        @foreach($pagos as $pago)
            <tr>
                ...
                <td style=" border: 1px solid black;">
                    ...
                    @elseif($pago->origen_type==\App\Models\FormularioLiquidacion::class)
                        {!! $pago->origen->cliente->nombre !!}
                    @elseif($pago->origen_type==\App\Models\Anticipo::class)
                        {!! $pago->origen->formularioLiquidacion->cliente->nombre !!}
                    @elseif($pago->origen_type==\App\Models\Prestamo::class)
                        {!! $pago->origen->cliente->nombre !!}
                    @endif
                </td>
                ...
                <td style=" border: 1px solid black;">
                    @if($pago->origen_type==\App\Models\Movimiento::class)
                            {!! $pago->origen->proveedor->empresa !!}
                    @elseif($pago->origen_type==\App\Models\FormularioLiquidacion::class)
                        {!! $pago->origen->cliente->cooperativa->razon_social !!}
                    @elseif($pago->origen_type==\App\Models\Anticipo::class)
                        {!! $pago->origen->formularioLiquidacion->cliente->cooperativa->razon_social !!}
                    @elseif($pago->origen_type==\App\Models\Prestamo::class)
                        {!! $pago->origen->cliente->cooperativa->razon_social !!}
                    @else
                        {!! $pago->origen->cooperativa->razon_social !!}
                    @endif
                </td>
