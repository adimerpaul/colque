    ...
    <table style=" border: 1px solid black;" class="table table-striped" id="kardex-tabla" name="kardex-tabla">
        ...
        @foreach($formularios as $formulario)
            <tr>
                ...
                <td style=" border: 1px solid black;" id="pesoNetoSecoTd" class="pesoNetoSecoTd">{{round($formulario->peso_seco,2)}}</td>
                <td style=" border: 1px solid black;" id="leySnTd" class="snTd">{{round($formulario->ley_sn,3)}}
                </td>
                <td style=" border: 1px solid black;" id="pesoFinoSnTd" class="snTd">
{{--                    @foreach($formulario->laboratorio_promedio as $lab)--}}
                    {{--                        @if($lab->simbolo=='Sn')--}}
                    {{round(($formulario->ley_sn * $formulario->peso_seco / 100),5)}}
                    {{--                    @endif--}}
                    {{--                    @endforeach--}}
                </td>
                <td style=" border: 1px solid black;" id="cotizacionSnTd" class="snTd">{{round($formulario->cotizacion_sn,3)}}</td>
...
{{--                <td style=" border: 1px solid black;" id="anticiposTd" class="anticiposTd">{{round($formulario->totales['total_anticipos'],2) }}</td>--}}
                ...
                <td style=" border: 1px solid black;" id="cuentasCobrarTd" class="cuentasCobrarTd">
                    {{round($formulario->total_cuenta_cobrar,2) }}
{{--                    {{round($formulario->totales['total_cuentas_cobrar'],2) }}--}}
                </td>
                ...
                <td id="puntosTd" style=" border: 1px solid black;">{{ round($formulario->puntos,2) }}</td>
...
<script>
    ...
    var table = document.getElementById("kardex-tabla"), sumaHumedad = 0, sumaLeySn = 0, sumaHumedadKg = 0, sumaPesoFinoSn = 0
    ...
    , sumaComision = 0, sumaCotizacionSn = 0, sumaPesoNetoSeco = 0,  sumaValorTonelada=0
    ...
    for(var i = 3; i < (table.rows.length - 1); i++) {
        ...
        sumaPesoFinoSn = parseFloat(sumaPesoFinoSn) + parseFloat(table.rows[i].cells[15].innerHTML.replace(/,/g, "."));
        sumaCajaSalud = parseFloat(sumaCajaSalud) + parseFloat(table.rows[i].cells[20].innerHTML.replace(/,/g, "."));
        sumaComibol = parseFloat(sumaComibol) + parseFloat(table.rows[i].cells[21].innerHTML.replace(/,/g, "."));
        sumaFencominNorpo = parseFloat(sumaFencominNorpo) + parseFloat(table.rows[i].cells[22].innerHTML.replace(/,/g, "."));
        sumaFencomin = parseFloat(sumaFencomin) + parseFloat(table.rows[i].cells[23].innerHTML.replace(/,/g, "."));
        sumaGastosAdmin = parseFloat(sumaGastosAdmin) + parseFloat(table.rows[i].cells[24].innerHTML.replace(/,/g, "."));
        sumaTransporte = parseFloat(sumaTransporte) + parseFloat(table.rows[i].cells[26].innerHTML.replace(/,/g, "."));
        sumaLeyes = parseFloat(sumaLeyes) + parseFloat(table.rows[i].cells[27].innerHTML.replace(/,/g, "."));
        sumaViaticos = parseFloat(sumaViaticos) + parseFloat(table.rows[i].cells[28].innerHTML.replace(/,/g, "."));
        // sumaCuentasCobrar = parseFloat(sumaCuentasCobrar) + parseFloat(table.rows[i].cells[32].innerHTML.replace(/,/g, "."));
        sumaAporteFundacion = parseFloat(sumaAporteFundacion) + parseFloat(table.rows[i].cells[34].innerHTML.replace(/,/g, "."));
        sumaTratamiento = parseFloat(sumaTratamiento) + parseFloat(table.rows[i].cells[36].innerHTML.replace(/,/g, "."));
        sumaLaboratorio = parseFloat(sumaLaboratorio) + parseFloat(table.rows[i].cells[37].innerHTML.replace(/,/g, "."));
        sumaPesaje = parseFloat(sumaPesaje) + parseFloat(table.rows[i].cells[38].innerHTML.replace(/,/g, "."));
        sumaComision = parseFloat(sumaComision) + parseFloat(table.rows[i].cells[39].innerHTML.replace(/,/g, "."));
        sumaCotizacionSn = parseFloat(sumaCotizacionSn) + (parseFloat(table.rows[i].cells[13].innerHTML.replace(/,/g, ".")) * parseFloat(table.rows[i].cells[16].innerHTML.replace(/,/g, ".")));
        sumaPesoNetoSeco = parseFloat(sumaPesoNetoSeco) + parseFloat(table.rows[i].cells[13].innerHTML.replace(/,/g, "."));
        sumaValorTonelada= parseFloat(sumaValorTonelada) + (parseFloat(table.rows[i].cells[13].innerHTML.replace(/,/g, ".")) * parseFloat(table.rows[i].cells[17].innerHTML.replace(/,/g, ".")));

    }
        sumaHumedad = (parseFloat("{{ $formularios->sum('humedad_kilo')}}")  / parseFloat("{{ $formularios->sum('peso_neto')}}"))*100;
        sumaCotizacionSn = parseFloat(sumaCotizacionSn) / parseFloat(sumaPesoNetoSeco);
    ...
    $(document).ready(function(){
        for(var i = 0; i < JSON.parse(campos).length; i++) {
            if(!JSON.parse(campos)[i].visible){
                if(JSON.parse(campos)[i].codigo == 'sn' || JSON.parse(campos)[i].codigo == 'retencionesDeLey'
