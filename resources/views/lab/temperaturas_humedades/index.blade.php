...
@section('content')
    <section class="content-header">
        <h1 class="pull-left">Temperaturas / Humedades <strong>{{$ambiente}}</strong></h1>
        <h1 class="pull-right">
            ...
            <a class="btn btn-success pull-right" style="margin-top: -10px;margin-bottom: 5px; margin-right: 5px"
               href="{{ route('rangos-mediciones.index') }}"
            >Rango</a>
        </h1>
        <br>
    </section>
    <div class="content">
        ...
        <div class="box box-primary">
            <div class="box-body">
                <div class="row">
                    <div class="col-sm-12">
                        {!! Form::open(['route' => 'temperaturas-humedades.index', 'method'=>'get']) !!}

                        {!! Form::hidden('ambiente', $ambiente, ['class' => 'form-control']) !!}
                ...
                @include('lab.temperaturas_humedades.table')
            </div>
        </div>
        <div class="text-center">
            {{ $temperaturas->appends($_GET)->links() }}
        </div>
    </div>
    {!! Form::open(['route' => 'temperaturas-humedades.store', 'id' => 'formularioModal']) !!}
    @include("lab.temperaturas_humedades.modal_registro")
    {!! Form::close() !!}

    {!! Form::open(['route' => 'actualizar-temperatura-humedad', 'id' => 'formularioModalEdit']) !!}
    @include("lab.temperaturas_humedades.modal_edicion")
    {!! Form::close() !!}
@endsection
...
@push('scripts')
    ...
    <script>
        ...
        $('#modalEdicion').on('show.bs.modal', function (event) {
            ...
            var ambiente = button.data('txtambiente')
            var valor = button.data('txtvalor')
            var tipo = button.data('txttipo')

            var modal = $(this)
            modal.find('.modal-body #idTemperatura').val(id);
        ...
        function exportarAPdf() {
            ...
            if (inicio == '' || fin == '') {
                ...
                window.open("/lab/get-temperaturas-pdf/"+"{{$ambiente}}/" + inicio + '/' + fin);
            }
        }

        function exportarAExcel() {
            var nombreArchivo = 'reporteTemperatura'+ "{{$ambiente}}_" + "_{{ $fecha_inicial }}" + '_AL_' + "{{ $fecha_final}}" + '.xls';
