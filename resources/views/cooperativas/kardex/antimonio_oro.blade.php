<div class="table-responsive">
    <table style=" border: 1px solid black;" class="table table-striped" id="kardex-tabla" name="kardex-tabla">
        ...
        <tr>
            ...
            <th colspan="46" style="text-align: center; border: 0px white !important">COLQUECHACA MINING LTDA.
        ...
        @foreach($formularios as $formulario)
            <tr>
                ...
                <td style=" border: 1px solid black;" id="pesoNetoHumedoTd"
                    class="pesoNetoHumedoTd">{{round($formulario->peso_neto,5)}}</td>
                <td style=" border: 1px solid black;" id="humedadPorcentajeTd"
                    class="humedadPorcentajeTd"> {{round($formulario->humedad_promedio,2)}}
                </td>
                <td style=" border: 1px solid black;" id="humedadKgTd"
                    class="humedadKgTd"> {{round($formulario->humedad_kilo,2)}}
                </td>
                <td style=" border: 1px solid black;" id="mermaTd"
                    class="mermaTd">{{round($formulario->merma_kg,2)}}</td>
                ...
                <td style=" border: 1px solid black;" id="cotizacionsbTd"
                    class="sbTd">{{round($formulario->cotizacion_sb,3)}}</td>
                ...
                <td style=" border: 1px solid black;"
                    class="auTd">{{round($formulario->cotizacion_au,3)}}</td>
                ...
                <td style=" border: 1px solid black;" id="anticiposTd"
                    class="anticiposTd">{{round($formulario->total_anticipo,2) }}</td>
                <td style=" border: 1px solid black;" id="cuentasCobrarTd"
                    class="cuentasCobrarTd">{{round($formulario->total_cuenta_cobrar,2) }}</td>



                <td style=" border: 1px solid black;" id="aporteFundacionTd"
                    class="aporteFundacionTd">{{round(($formulario->aporte_fundacion),2) }}</td>
                <td style=" border: 1px solid black;" id="saldoFavorTd"
                    class="saldoFavorTd">{{round(($formulario->saldo_favor),2) }}</td>

                <td style=" border: 1px solid black;" id="tratamientoTd"
                    class="tratamientoTd">{{round(($formulario->peso_bruto * $formulario->costo->tratamiento * 6.97),2) }}</td>

                <td style=" border: 1px solid black;" id="laboratorioTd"
                    class="laboratorioTd">{{round(($formulario->costo->laboratorio + $formulario->costo->dirimicion),2) }}</td>
                <td style=" border: 1px solid black;" id="pesajeTd"
                    class="pesajeTd">{{round($formulario->costo_pesaje,2) }}</td>
                <td style=" border: 1px solid black;" id="comisionTd"
                    class="comisionTd">{{round(($formulario->peso_seco * $formulario->costo->comision * 6.86),2) }}</td>
        ...
        <tr>
            ...
            <td style=" border: 1px solid black;" id="pesoNetoHumedoTotal">
                <b> {{ round($formularios->sum('peso_neto'),5)}}</b></td>
            ...
            <td style=" border: 1px solid black;font-weight: bold"
                id="mermaTotal">{{ round($formularios->sum('merma_kg'),2)}}</td>
...
<script>
    ...
    var table = document.getElementById("kardex-tabla"), sumaHumedad = 0, sumaLeySb = 0,
        sumaHumedadKg = 0, sumaPesoFinoSb = 0
        , sumaPesoFinoAu = 0, sumaTratamiento = 0,
        sumaLaboratorio = 0
        , sumaComision = 0, sumaCotizacionSb = 0, sumaPesoNetoSeco = 0,
         sumaValorTonelada=0
        , sumaCuentasCobrar = 0, sumaLeyAu = 0, sumaCotizacionAu = 0, sumaPesaje = 0, sumaAporteFundacion = 0,
    ...
    for (var i = 3; i < (table.rows.length - 1); i++) {
        ...
        sumaPesoFinoSb = parseFloat(sumaPesoFinoSb) + parseFloat(table.rows[i].cells[14].innerHTML.replace(/,/g, "."));
        sumaPesoFinoAu = parseFloat(sumaPesoFinoAu) + parseFloat(table.rows[i].cells[17].innerHTML.replace(/,/g, "."));

        // sumaCuentasCobrar = parseFloat(sumaCuentasCobrar) + parseFloat(table.rows[i].cells[26+nroDescRet+nroBon+nroBonAcu].innerHTML.replace(/,/g, "."));
        sumaAporteFundacion = parseFloat(sumaAporteFundacion) + parseFloat(table.rows[i].cells[28+nroDescRet+nroBon+nroBonAcu].innerHTML.replace(/,/g, "."));
        sumaTratamiento = parseFloat(sumaTratamiento) + parseFloat(table.rows[i].cells[30+nroDescRet+nroBon+nroBonAcu].innerHTML.replace(/,/g, "."));
        sumaLaboratorio = parseFloat(sumaLaboratorio) + parseFloat(table.rows[i].cells[31+nroDescRet+nroBon+nroBonAcu].innerHTML.replace(/,/g, "."));
        sumaPesaje = parseFloat(sumaPesaje) + parseFloat(table.rows[i].cells[32+nroDescRet+nroBon+nroBonAcu].innerHTML.replace(/,/g, "."));
        sumaComision = parseFloat(sumaComision) + parseFloat(table.rows[i].cells[33+nroDescRet+nroBon+nroBonAcu].innerHTML.replace(/,/g, "."));
        sumaCotizacionSb = parseFloat(sumaCotizacionSb) + (parseFloat(table.rows[i].cells[12].innerHTML.replace(/,/g, ".")) * parseFloat(table.rows[i].cells[15].innerHTML.replace(/,/g, ".")));
        sumaPesoNetoSeco = parseFloat(sumaPesoNetoSeco) + parseFloat(table.rows[i].cells[12].innerHTML.replace(/,/g, "."));
        sumaCotizacionAu = parseFloat(sumaCotizacionAu) + (parseFloat(table.rows[i].cells[12].innerHTML.replace(/,/g, ".")) * parseFloat(table.rows[i].cells[18].innerHTML.replace(/,/g, ".")));
        sumaValorTonelada= parseFloat(sumaValorTonelada) + (parseFloat(table.rows[i].cells[12].innerHTML.replace(/,/g, ".")) * parseFloat(table.rows[i].cells[19].innerHTML.replace(/,/g, ".")));
    ...
    $(document).ready(function () {
        for (var i = 0; i < JSON.parse(campos).length; i++) {
            if (!JSON.parse(campos)[i].visible) {
                if (JSON.parse(campos)[i].codigo == 'au'
                    || JSON.parse(campos)[i].codigo == 'sb' || JSON.parse(campos)[i].codigo == 'retencionesDeLey'
