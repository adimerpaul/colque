<div class="table-responsive">
    <table style=" border: 1px solid black;" class="table table-striped" id="kardex-tabla" name="kardex-tabla">
        <thead>
        <tr>
            <th colspan="21" style="text-align: center; border: 0px white !important">COMPOSITO
                <br>LOTE COLQUECHACA: {{$venta->lote}}
                <br>CÓDIGO: {{$venta->letra}}
                <br>PRODUCTO: COMPLEJO
                <br><br>
            </th>
        ...
        <tr style="background-color: #042E44; color: white;">
            ...
            <th colspan="3" style=" border: 1px solid #999999; text-align: center" id="pb" class="pb">PB</th>
        ...
        <tr style="background-color: #042E44; color: white">
            ...
            <th style=" border: 1px solid #999999;" id="pb" class="pb">%Pb</th>
            <th style=" border: 1px solid #999999;" id="pb" class="pb">Peso Fino (Kg)</th>
            <th style=" border: 1px solid #999999;" id="pb" class="pb">C. Diaria</th>
        ...
        @foreach($formularios as $formulario)
            <tr>
                ...
                <td style=" border: 1px solid black;" id="cotizacionPbTd"
                    class="pbTd">@if(round($formulario->cotizacion_pb,2)!=0){{number_format($formulario->cotizacion_pb,2, ',', '')}}@endif</td>
        ...
        @foreach($ingenios as $ingenio)
            <tr>
                ...
                <td style=" border: 1px solid black;" name="ley_pb"
                >@if(round($ingenio->ley_pb,2)!=0){{ number_format($ingenio->ley_pb,2, ',', '') }}@endif
                </td>
                <td style=" border: 1px solid black;">@if(round($ingenio->peso_fino_pb,2)!=0){{ number_format($ingenio->peso_fino_pb,2, ',', '') }}@endif</td>
                <td style=" border: 1px solid black;" name="cotizacion_pb"
                >@if(round($ingenio->cotizacion_pb,2)!=0){{ number_format($ingenio->cotizacion_pb,2, ',', '') }}@endif
                </td>
        ...
        <tr style="background-color: #042E44; color: white">
            ...
            <td rowspan="3" style=" border: 1px solid #999999; font-weight: bold" id="cotizacionZnTotal"
                class="znTotal"></td>
            <td rowspan="3" style=" border: 1px solid #999999; font-weight: bold" id="leyPbTotal" class="pbTotal"></td>
            <td rowspan="3" style=" border: 1px solid #999999; font-weight: bold" id="pesoFinoPbTotal"
                class="pbTotal"></td>
            <td rowspan="3" style=" border: 1px solid #999999; font-weight: bold" id="cotizacionPbTotal"
                class="pbTotal"></td>
        ...
        <tr v-for="(concentrado, index) in concentrados" :key="index">
            ...
            <td style=" border: 1px solid black;"
                :contenteditable="concentrado.habilitado_ingenio &&{{ ($venta->es_escritura AND \App\Patrones\Permiso::esComercial()) ? 'true': 'false'}}"
                :value="concentrado.ley_pb" :id="concentrado.id" name="ley_pb"
                v-on:blur="actualizarConcentrado($event)">@{{ agregarComa(concentrado.ley_pb) }}
            </td>
            <td style=" border: 1px solid black;">@{{ agregarComa(concentrado.peso_fino_pb) }}</td>
            <td style=" border: 1px solid black;"
                :contenteditable="concentrado.habilitado_ingenio &&{{ ($venta->es_escritura AND \App\Patrones\Permiso::esComercial()) ? 'true': 'false'}}"
                :value="concentrado.cotizacion_pb" :id="concentrado.id" name="cotizacion_pb"
                v-on:blur="actualizarConcentrado($event)">@{{ agregarComa(concentrado.cotizacion_pb) }}
            </td>
            <td style=" border: 1px solid black;"
                :contenteditable="concentrado.habilitado_ingenio &&{{ ($venta->es_escritura AND \App\Patrones\Permiso::esComercial()) ? 'true': 'false'}}"
        ...
        <tr style="background-color: #042E44; color: white">
            ...
            <td style=" border: 1px solid #999999; font-weight: bold">@{{ leyPbSuma }}</td>
            <td style=" border: 1px solid #999999; font-weight: bold">@{{ pesoFinoPbSuma }}</td>
            <td style=" border: 1px solid #999999; font-weight: bold">@{{ cotizacionPbSuma }}</td>
...
<script type="application/javascript">
    ...
    var table = document.getElementById("kardex-tabla"), sumaHumedad = 0, sumaLeyZn = 0,
        ...
        , sumaComisionExterna = 0, sumaLaboratorio = 0
        , sumaPromocion = 0, sumaCotizacionZn = 0, sumaPesoNetoSeco = 0, sumaPesoNetoSecoZn = 0, sumaPesoNetoSecoPb = 0
        , sumaPesoFinoPb = 0, sumaLeyPb = 0, sumaCotizacionPb = 0, sumaBonoTransporte = 0, sumaBonoCliente = 0
    ...
    for (var i = 0; i < (tbody.rows.length - 3); i++) {
        ...
        sumaNetoVenta = parseFloat(sumaNetoVenta) + parseFloat(tbody.rows[i].cells[16].innerHTML.replace(/,/g, "."));
        sumaRegalia = parseFloat(sumaRegalia) + parseFloat(tbody.rows[i].cells[17].innerHTML.replace(/,/g, "."));

        //sumaPesoFinoZn = parseFloat(sumaPesoFinoZn) + parseFloat(tbody.rows[i].cells[8].innerHTML.replace(/,/g, "."));
        ...
        //sumaPesoFinoPb = parseFloat(sumaPesoFinoPb) + parseFloat(tbody.rows[i].cells[11].innerHTML.replace(/,/g, "."));
        sumaPesoFinoPb = parseFloat(sumaPesoFinoPb) +
            (tbody.rows[i].cells[11].innerHTML.trim() == '' ? parseFloat(0.0) : parseFloat(tbody.rows[i].cells[11].innerHTML.replace(/,/g, ".")));
        sumaPesoFinoAg = parseFloat(sumaPesoFinoAg) + parseFloat(tbody.rows[i].cells[14].innerHTML.replace(/,/g, "."));
        sumaBonoTransporte = parseFloat(sumaBonoTransporte) + parseFloat(tbody.rows[i].cells[18].innerHTML.replace(/,/g, "."));
        sumaBonoCliente = parseFloat(sumaBonoCliente) + parseFloat(tbody.rows[i].cells[19].innerHTML.replace(/,/g, "."));

        sumaComisionExterna = parseFloat(sumaComisionExterna) + parseFloat(tbody.rows[i].cells[20].innerHTML.replace(/,/g, "."));
        sumaLaboratorio = parseFloat(sumaLaboratorio) + parseFloat(tbody.rows[i].cells[21].innerHTML.replace(/,/g, "."));
        sumaPesaje = parseFloat(sumaPesaje) + parseFloat(tbody.rows[i].cells[22].innerHTML.replace(/,/g, "."));
        sumaProProductor = parseFloat(sumaProProductor) + parseFloat(tbody.rows[i].cells[23].innerHTML.replace(/,/g, "."));
        sumaPromocion = parseFloat(sumaPromocion) + parseFloat(tbody.rows[i].cells[24].innerHTML.replace(/,/g, "."));

        //sumaCotizacionZn = parseFloat(sumaCotizacionZn) + (parseFloat(tbody.rows[i].cells[6].innerHTML.replace(/,/g, ".")) * parseFloat(tbody.rows[i].cells[9].innerHTML.replace(/,/g, ".")));
        ...
        //sumaCotizacionPb = parseFloat(sumaCotizacionPb) + (parseFloat(tbody.rows[i].cells[6].innerHTML.replace(/,/g, ".")) * parseFloat(tbody.rows[i].cells[12].innerHTML.replace(/,/g, ".")));
        sumaCotizacionPb = parseFloat(sumaCotizacionPb) + (
            tbody.rows[i].cells[12].innerHTML.trim() == '' ? parseFloat(0.0) :
                (parseFloat(tbody.rows[i].cells[6].innerHTML.replace(/,/g, ".")) * parseFloat(tbody.rows[i].cells[12].innerHTML.replace(/,/g, "."))));
        sumaPesoNetoSecoPb = parseFloat(sumaPesoNetoSecoPb) + (tbody.rows[i].cells[12].innerHTML.trim() == '' ? parseFloat(0.0) : parseFloat(tbody.rows[i].cells[6].innerHTML.replace(/,/g, ".")));

        sumaPesoNetoSeco = parseFloat(sumaPesoNetoSeco) + parseFloat(tbody.rows[i].cells[6].innerHTML.replace(/,/g, "."));
        sumaCotizacionAg = parseFloat(sumaCotizacionAg) + (parseFloat(tbody.rows[i].cells[6].innerHTML.replace(/,/g, ".")) * parseFloat(tbody.rows[i].cells[15].innerHTML.replace(/,/g, ".")));
    ...
    document.getElementById("leyPbTotal").innerHTML = agregarComma(sumaLeyPb.toFixed(2));
    document.getElementById("pesoFinoZnTotal").innerHTML = agregarComma(sumaPesoFinoZn.toFixed(2));
    document.getElementById("pesoFinoPbTotal").innerHTML = agregarComma(sumaPesoFinoPb.toFixed(2));
