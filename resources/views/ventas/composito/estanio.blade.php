<div class="table-responsive">
    <table style=" border: 1px solid black;" class="table table-striped" id="kardex-tabla" name="kardex-tabla">
        ...
        <tr style="background-color: #042E44; color: white;">
            ...
            <th colspan="3" style=" border: 1px solid #999999; text-align: center" id="sn" class="sn">SN</th>
        ...
        <tr style="background-color: #042E44; color: white">
            <th style=" border: 1px solid #999999;" id="sn" class="sn">%Sn</th>
            <th style=" border: 1px solid #999999;" id="sn" class="sn">Peso Fino (Kg)</th>
            <th style=" border: 1px solid #999999;" id="sn" class="sn">C. Diaria</th>
        </tr>
        </thead>
        <tbody id="tbody">
        <?php
        ...
        @foreach($formularios as $formulario)
            <tr>
                ...
                <td style=" border: 1px solid black; background-color: #00838F; color:white" id="leySnTd" class="snTd">
                    @foreach($formulario->laboratorio_promedio as $lab)
                        @if($lab->simbolo=='Sn') {{number_format($lab->promedio,2, ',', '')}} @endif
                    @endforeach
                </td>
                <td style=" border: 1px solid black;" id="pesoFinoSnTd"
                    class="snTd"> @foreach($formulario->laboratorio_promedio as $lab)
                        @if($lab->simbolo=='Sn'){{number_format(($lab->promedio * $formulario->peso_seco / 100),2, ',', '')}}  @endif
                    @endforeach
                </td>
                <td style=" border: 1px solid black;" id="cotizacionSnTd"
                    class="snTd">{{number_format($formulario->cotizacion_sn,2, ',', '')}}</td>
                ...
                <td style=" border: 1px solid black;" id="bonoClienteTd"
                    class="bonoClienteTd">{{number_format(( $formulario->id==6554? 5392:  $formulario->bonificaciones_descuentos_bob['productor']),2, ',', '') }}</td>
{{--HARCCODE--}}
        ...
        @foreach($ingenios as $ingenio)
            <tr>
                ...
                <td style=" border: 1px solid black;" name="ley_sn"
                >{{ number_format($ingenio->ley_sn,2, ',', '') }}
                </td>
                <td style=" border: 1px solid black;">{{ number_format($ingenio->peso_fino_sn,2, ',', '') }}</td>
                <td style=" border: 1px solid black;" name="cotizacion_sn"
                >{{ number_format($ingenio->cotizacion_sn,2, ',', '') }}
                </td>
        ...
        <tr style="background-color: #042E44; color: white">
            ...
            <td rowspan="3" style=" border: 1px solid #999999; font-weight: bold" id="leySnTotal" class="snTotal"></td>
            <td rowspan="3" style=" border: 1px solid #999999; font-weight: bold" id="pesoFinoSnTotal"
                class="snTotal"></td>
            <td rowspan="3" style=" border: 1px solid #999999; font-weight: bold" id="cotizacionSnTotal"
                class="snTotal"></td>
        ...
        <tr v-for="(concentrado, index) in concentrados" :key="index">
            ...
            <td style=" border: 1px solid black;"
                :contenteditable="concentrado.habilitado_ingenio &&{{ ($venta->es_escritura AND \App\Patrones\Permiso::esComercial())  ? 'true': 'false'}}"
                :value="concentrado.ley_sn" :id="concentrado.id" name="ley_sn"
                v-on:blur="actualizarConcentrado($event)">@{{ agregarComa(concentrado.ley_sn) }}
            </td>
            <td style=" border: 1px solid black;">@{{ agregarComa(concentrado.peso_fino_sn) }}</td>
            <td style=" border: 1px solid black;"
                :contenteditable="concentrado.habilitado_ingenio &&{{ ($venta->es_escritura AND \App\Patrones\Permiso::esComercial())  ? 'true': 'false'}}"
                :value="concentrado.cotizacion_sn" :id="concentrado.id" name="cotizacion_sn"
                v-on:blur="actualizarConcentrado($event)">@{{ agregarComa(concentrado.cotizacion_sn) }}
            </td>
        ...
        <tr style="background-color: #042E44; color: white">
            ...
            <td style=" border: 1px solid #999999; font-weight: bold">@{{ leySnSuma }}</td>
            <td style=" border: 1px solid #999999; font-weight: bold">@{{ pesoFinoSnSuma }}</td>
            <td style=" border: 1px solid #999999; font-weight: bold">@{{ cotizacionSnSuma }}</td>
...
<script type="application/javascript">
    ...
    var table = document.getElementById("kardex-tabla"), sumaHumedad = 0, sumaLeySn = 0,
        sumaPesoFinoSn = 0, costoMercaderia = 0
        , sumaComisionExterna = 0, sumaLaboratorio = 0, sumaBonoTransporte = 0, sumaBonoCliente = 0
        , sumaPromocion = 0, sumaCotizacionSn = 0, sumaPesoNetoSeco = 0, sumaProProductor = 0
        , sumaPesaje = 0
    ...
    for (var i = 0; i < (tbody.rows.length - 3); i++) {
        ...
        sumaPesoFinoSn = parseFloat(sumaPesoFinoSn) + parseFloat(tbody.rows[i].cells[8].innerHTML.replace(/,/g, "."));
        ...
        sumaCotizacionSn = parseFloat(sumaCotizacionSn) + (parseFloat(tbody.rows[i].cells[6].innerHTML.replace(/,/g, ".")) * parseFloat(tbody.rows[i].cells[9].innerHTML.replace(/,/g, ".")));
    ...
    document.getElementById("leySnTotal").innerHTML = agregarComma(sumaLeySn.toFixed(2));
    document.getElementById("pesoFinoSnTotal").innerHTML = agregarComma(sumaPesoFinoSn.toFixed(2));
    ...
    document.getElementById("cotizacionSnTotal").innerHTML = agregarComma(sumaCotizacionSn.toFixed(2));
