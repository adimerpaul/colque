<div class="table-responsive">
    <table style=" border: 1px solid black;" class="table table-striped" id="kardex-tabla" name="kardex-tabla">
        ...
        <tr v-for="(concentrado, index) in concentrados" :key="index">
            ...
            {{--            <td style=" border: 1px solid black;" :contenteditable="concentrado.habilitado_ingenio &&{{ ($venta->es_escritura AND \App\Patrones\Permiso::esComercial()) ? 'true': 'false'}}" :value="concentrado.retenciones" :id="concentrado.id" name="retenciones"--}}
            {{--                v-on:blur="actualizarConcentrado($event)">@{{ agregarComa(concentrado.retenciones) }}</td>--}}
...
<script type="application/javascript">
    ...
    var table = document.getElementById("kardex-tabla"), sumaHumedad = 0, sumaLeyPb = 0,
        sumaPesoFinoPb = 0
        , sumaPesoFinoAg = 0, costoMercaderia = 0
        , sumaComisionExterna = 0, sumaLaboratorio = 0, sumaBonoTransporte = 0, sumaBonoCliente = 0
        , sumaPromocion = 0, sumaCotizacionPb = 0, sumaPesoNetoSeco = 0, sumaPesoNetoSecoPb = 0
    ...
    for (var i = 0; i < (tbody.rows.length - 3); i++) {
        ...
        //sumaCotizacionPb = parseFloat(sumaCotizacionPb) + (parseFloat(tbody.rows[i].cells[6].innerHTML.replace(/,/g, ".")) * parseFloat(tbody.rows[i].cells[9].innerHTML.replace(/,/g, ".")));
        ...
        sumaPesoNetoSecoPb = parseFloat(sumaPesoNetoSecoPb) + (tbody.rows[i].cells[9].innerHTML.trim() == '' ? parseFloat(0.0) : parseFloat(tbody.rows[i].cells[6].innerHTML.replace(/,/g, ".")));
