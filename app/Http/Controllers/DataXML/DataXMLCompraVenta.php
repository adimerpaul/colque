...
class DataXMLCompraVenta
{
    ...
    function makeHead($xml, $cuf, $cufd, $numeroFactura, $fechaActual)
    {
        return [
            ...
            "nombreRazonSocial" =>  trim($xml->numero_documento) === "0" ? "Control Tributario" : str_replace("&","&amp;",trim($xml->nombre_razon_social)),
            "codigoTipoDocumentoIdentidad" => strlen(trim($xml->numero_documento)) > 8 ? 5 : 1,
            "numeroDocumento" => trim($xml->numero_documento) === "0"  ? "99002" : $xml->numero_documento,
            "complemento" => null,
