...
class GenerateConciliacionPDF
{
    public static function generateHTML($xml,$cuf,$complemento): string
    {
        ...
        $literal = $formatter->toInvoice((float)$xml->cabecera->montoTotalConciliado, 2, 'Bolivianos');
        $detalles = "";
        $date=date_create($xml->cabecera->fechaEmisionFactura);
        $xml->cabecera->fechaEmisionFactura= date_format($date,"d/m/Y h:i A");
        foreach ($xml->detalleOriginal as $d) {
        ...
        foreach ($xml->detalleConciliacion as $d) {
            $detallesConciliacion .= '
                <tr>
                    <td class="border">' . $d->codigoProducto . '</td>
                    <td class="border">' . $d->descripcion . '</td>
                    <td class="border right">' . $d->montoOriginal . '</td>
                    <td class="border right">' . $d->montoFinal . '</td>
                    <td class="border right">' . $d->montoConciliado . '</td>
                </tr>
            ';
        }
        if ($xml->cabecera->creditoFiscalIva == 0) {
            $textoCredito = "DÉBITO";
            $montoCredito = $xml->cabecera->debitoFiscalIva;
        } else {
...
<table width="100%"  class="collapse" >
            ...
            <table>
                <tr>
                    <td valign=top width="130px">
                        ...
                        <div class="bold">Nº NOTA</div>
                        <div class="bold">CÓD. AUTORIZACIÓN</div>
                    </td>
                    <td>
                        <div>' . $xml->cabecera->nitEmisor . '</div>
                        <div>' . $xml->cabecera->numeroNotaConciliacion . '</div>
    ...
    <tr valign="bottom">
        ...
        <tr>
        <td colspan="3">
        <h5>DATOS DE LA CONCILIACIÓN</h5>
            <table width="100%" class="collapse">
                <tr class="background" >
                    <th class="border" width="150px" >CÓDIGO PRODUCTO</th>
                    <th class="border">DESCRIPCIÓN</th>
                    <th class="border" width="50px">MONTO ORIGINAL</th>
                    <th class="border">MONTO FINAL</th>
                    <th class="border"  width="100px">MONTO CONCILIADO</th>
                </tr>
                ' . $detallesConciliacion . '
                <tr>
                    <td colspan="2"></td>
                    <td class="border right text-h5 bold background" colspan="2">MONTO TOTAL CONCILIADO Bs</td>
                    <td class="border right">' . $xml->cabecera->montoTotalConciliado . '</td>
                </tr>
                <tr>
                    <td colspan="2"></td>
                    <td class="border right  text-h5 bold background" colspan="2">' . $textoCredito . ' FISCAL IVA Bs</td>
                    <td class="border right">' . $montoCredito . '</td>
                </tr>
            </table>
        </td>
    </tr>
